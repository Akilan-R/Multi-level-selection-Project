...
def main():
    ...
    C_value_dictionary_for_model = {'C1': -0.3, 'C2': 1.3}
...
class run_simulation_class:
    ...
    def run_simulation_method(self):
            ...
            while(True):
                ...
                population_growth_for_n_groups_instance = population_growth_for_n_groups_class(self.parameters_object_for_run_simulation, list_of_groups_for_calculating_population_growth_over_time)

                population_growth_for_n_groups_instance.population_growth_of_n_groups_over_time()
                population_growth_for_n_groups_instance.find_pooled_groups_dictionary()

                for trait in ["C1", "C2"]:

                    net_fitness_for_trait = population_growth_for_n_groups_instance.calcualate_net_fitness_for_a_gien_trait_at_given_time_step(trait)
                    print("net_fitness_for_trait =", net_fitness_for_trait, "at time step",time_step_number)
